...
function wpc_plugin_action_links($links, $file) {
    ...
    if ($file == $this_plugin) {
        $settings_link = '<a href="' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=wpc-settings-admin">'.__('Settings', 'wp-cloudy').'</a>';
...
function init_metabox(){
	add_meta_box('wpcloudy_basic', 'WP Cloudy Settings - <a href="'.get_bloginfo('url').'/wp-admin/options-general.php?page=wpc-settings-admin">'.__('WP Cloudy global settings','wp-cloudy').'</a>', 'wpcloudy_basic', 'wpc-weather', 'advanced');
	...
	echo '<div id="wpcloudy-tabs">
			...
			<div id="tabs-1">
				<p>
					...
					<input id="wpcloudy_city_meta" data_appid="'.get_admin_api_key2().'" class="cities typeahead" type="text" name="wpcloudy_city" placeholder="'.__('Enter your city','wp-cloudy').'" value="'.$wpcloudy_city.'" />
				</p>
				<p>
				...
				<p>
					<label for="wpcloudy_state_name_meta">'. __( 'State?', 'wp-cloudy' ) .'</label>
			...
			<div id="tabs-2">
				...
				<p>
					...
					<br />
					<span class="dashicons dashicons-editor-help"></span><a href="'.get_bloginfo('url').'/wp-admin/options-general.php" target="_blank">'.__('Make sure you have properly set the date of your site in WordPress settings.','wp-cloudy').'</a>
				</p>
				<p class="forecast">
					'. __( '16-Day Forecast', 'wp-cloudy' ) .'
				</p>
				<p>
					<label for="wpcloudy_forecast_meta">
						<input type="checkbox" name="wpcloudy_forecast" id="wpcloudy_forecast_meta" value="yes" '. checked( $wpcloudy_forecast, 'yes', false ) .' />
							'. __( '16-Day Forecast? (today + 15 days max)', 'wp-cloudy' ) .'
					</label>
				</p>
				<p>
					...
					<select name="wpcloudy_forecast_nd">
						...
						<option ' . selected( '6', $wpcloudy_forecast_nd, false ) . ' value="6">'. __( '6 days', 'wp-cloudy' ) .'</option>
						<option ' . selected( '7', $wpcloudy_forecast_nd, false ) . ' value="7">'. __( '7 days', 'wp-cloudy' ) .'</option>
						<option ' . selected( '8', $wpcloudy_forecast_nd, false ) . ' value="8">'. __( '8 days', 'wp-cloudy' ) .'</option>
						<option ' . selected( '9', $wpcloudy_forecast_nd, false ) . ' value="9">'. __( '9 days', 'wp-cloudy' ) .'</option>
						<option ' . selected( '10', $wpcloudy_forecast_nd, false ) . ' value="10">'. __( '10 days', 'wp-cloudy' ) .'</option>
						<option ' . selected( '11', $wpcloudy_forecast_nd, false ) . ' value="11">'. __( '11 days', 'wp-cloudy' ) .'</option>
						<option ' . selected( '12', $wpcloudy_forecast_nd, false ) . ' value="12">'. __( '12 days', 'wp-cloudy' ) .'</option>
						<option ' . selected( '13', $wpcloudy_forecast_nd, false ) . ' value="13">'. __( '13 days', 'wp-cloudy' ) .'</option>
						<option ' . selected( '14', $wpcloudy_forecast_nd, false ) . ' value="14">'. __( '14 days', 'wp-cloudy' ) .'</option>
						<option ' . selected( '15', $wpcloudy_forecast_nd, false ) . ' value="15">'. __( '15 days', 'wp-cloudy' ) .'</option>
					</select>
				</p>
				<p>
...
function wpc_get_my_weather_id($attr) {
	...
	$wpcloudy_css3_anims      					= get_bypass_disable_css3_anims($wpc_id);
	$wpcloudy_map           					= get_bypass_map($wpc_id); 
	...
	return '<div id="wpc-weather-id-'.$wpc_id.'" class="wpc-weather-id" data_id="'.$wpc_id.'" data_post_id="'.get_the_ID().'" data_map="'.$wpcloudy_map.'" data_detect_geolocation="'.$wpc_detectGeolocation.'" data_manual_geolocation="'.$wpc_manualGeolocation.'" data_wpc_lat="'.$wpc_posLat.'" data_wpc_lon="'.$wpc_posLon.'" data_wpc_city_id="'.$wpc_posCityId.'" data_wpc_city_name="'.$wpc_posCityName.'" data_custom_font="'.wpc_css_webfont($attr).'"></div>';	
}
...
function wpc_get_my_weather($attr) {
	...
	if ( isset( $_POST['wpc_param'] ) ) {
		...
		$wpcloudy_unit                				= get_bypass_unit($attr);
		$wpcloudy_map_height            			= get_bypass_map_height($attr);
		$wpcloudy_map_opacity          				= get_bypass_map_opacity($attr);
		$wpcloudy_map_zoom              			= get_bypass_map_zoom($attr);
		$wpcloudy_map_zoom_wheel          			= get_bypass_map_zoom_wheel($attr);
		$wpcloudy_map_stations            			= get_bypass_map_layers_stations($attr);
		$wpcloudy_map_clouds            			= get_bypass_map_layers_clouds($attr);
		$wpcloudy_map_precipitation         		= get_bypass_map_layers_precipitation($attr);
		$wpcloudy_map_snow              			= get_bypass_map_layers_snow($attr);
		$wpcloudy_map_wind              			= get_bypass_map_layers_wind($attr);
		$wpcloudy_map_temperature         			= get_bypass_map_layers_temperature($attr);
		$wpcloudy_map_pressure            			= get_bypass_map_layers_pressure($attr);
		$wpcloudy_meta_border_color         		= get_bypass_color_border($attr);
		$wpcloudy_meta_bg_color           			= get_bypass_color_background($attr);
		$wpcloudy_meta_text_color         			= get_bypass_color_text($attr);
		$wpcloudy_date_format          				= get_bypass_date($attr);
		$wpcloudy_sunrise_sunset          			= get_bypass_display_sunrise_sunset($attr);
		$wpcloudy_display_temp_unit         		= get_bypass_display_temp_unit($attr);
		$wpcloudy_display_length_days_names     	= get_bypass_length_days_names($attr);
		...
		$wpcloudy_forecast_nd     					= get_bypass_forecast_nd($attr); 
		$wpcloudy_forecast_precipitation			= get_bypass_forecast_precipitation($attr); 
		...
		$wpcloudy_date_temp       					= get_bypass_display_date_temp($attr);
		$wpcloudy_display_fluid       				= get_bypass_display_fluid($attr);
		$wpcloudy_display_fluid_width  				= get_bypass_display_fluid_width($attr);
        ...
        if( $wpcloudy_enable_geolocation == 'yes' && $_COOKIE['wpc-detectGeolocation']=='1' && $wpcloudy_enable_geolocation_custom_field != 'yes' ) { 
        	...
        	$myweather_current = @simplexml_load_string(wp_remote_fopen("http://api.openweathermap.org/data/2.5/weather?lat=$wpcloudy_lat&lon=$wpcloudy_lon&mode=xml&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key"));
        ...
        elseif( $wpcloudy_enable_geolocation == 'yes' && $_COOKIE['wpc-manualGeolocation']=='1' && $wpcloudy_enable_geolocation_custom_field != 'yes' )  {
        	$use_errors = libxml_use_internal_errors(true);
        	$myweather_current = @simplexml_load_string(wp_remote_fopen("http://api.openweathermap.org/data/2.5/weather?id=$wpcloudy_select_city_id&mode=xml&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key"));
        	if (!$myweather_current) {
          		_e('Unable to retrieve weather data','wp-cloudy');
			}
          	libxml_clear_errors();
	        ...
	        if ($wpc_advanced_set_disable_cache == '1') {
	        	$use_errors = libxml_use_internal_errors(true);
	          	$myweather_current = @simplexml_load_string(wp_remote_fopen("http://api.openweathermap.org/data/2.5/weather?q=$wpcloudy_city,$wpcloudy_state_name,$wpcloudy_country_code&mode=xml&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key"));
	          	if (!$myweather_current) {
	          		_e('Unable to retrieve weather data','wp-cloudy');
				}
	          	libxml_clear_errors();
		        ...
		        && false === ( $myweather_current = @simplexml_load_string(get_transient( 'myweather_current_'.$wpc_id.'_'.$wpc_get_the_ID ) ) ) )  {            
			        $use_errors = libxml_use_internal_errors(true);
			        $myweather_current = wp_remote_fopen("http://api.openweathermap.org/data/2.5/weather?q=$wpcloudy_custom_field_city_value,$wpcloudy_custom_field_country_value&mode=xml&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key");
			    	if ($myweather_current) {
						set_transient( 'myweather_current_'.$wpc_id.'_'.$wpc_get_the_ID, (string)$myweather_current, $wpc_advanced_set_cache_time * MINUTE_IN_SECONDS );
						$myweather_current = @simplexml_load_string(get_transient( 'myweather_current_'.$wpc_id.'_'.$wpc_get_the_ID ) );
					}
		          	libxml_clear_errors();
					libxml_use_internal_errors($use_errors);
	        	}
				...
				&& false === ( $myweather_current = @simplexml_load_string(get_transient( 'myweather_current_'.$wpc_id.'_'.$wpc_get_the_ID ) ) ) )  {            
			        $use_errors = libxml_use_internal_errors(true);
			        $myweather_current = wp_remote_fopen("http://api.openweathermap.org/data/2.5/weather?lat=$wpcloudy_custom_field_lat_value&lon=$wpcloudy_custom_field_lon_value&mode=xml&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key"); 
			        if ($myweather_current) {
						set_transient( 'myweather_current_'.$wpc_id.'_'.$wpc_get_the_ID, (string)$myweather_current, $wpc_advanced_set_cache_time * MINUTE_IN_SECONDS );
			        	$myweather_current = @simplexml_load_string(get_transient( 'myweather_current_'.$wpc_id.'_'.$wpc_get_the_ID ) );
					}
				...
				//Geolocation OFF / Custom Field OFF
				...
				&& false === ( $myweather_current = @simplexml_load_string(get_transient( 'myweather_current_'.$wpc_id ) ) ) ) {  
		            $use_errors = libxml_use_internal_errors(true);
		            $myweather_current = wp_remote_fopen("http://api.openweathermap.org/data/2.5/weather?q=$wpcloudy_city,$wpcloudy_state_name,$wpcloudy_country_code&mode=xml&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key");
		            if (!$myweather_current) {
		            }else {
        ...
        if( $wpcloudy_hour_forecast && !$wpcloudy_hour_forecast_nd =='' ) {
          	if( $wpcloudy_enable_geolocation == 'yes' && $_COOKIE['wpc-detectGeolocation']=='1' && $wpcloudy_enable_geolocation_custom_field != 'yes' ) {
            	$use_errors = libxml_use_internal_errors(true);
            	$myweather = @simplexml_load_string(wp_remote_fopen("http://api.openweathermap.org/data/2.5/forecast/weather?lat=$wpcloudy_lat&lon=$wpcloudy_lon&mode=xml&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key"));
            	if (!$myweather) {
          			_e('Unable to retrieve weather data','wp-cloudy');
				}
				libxml_clear_errors();
          	...
          	elseif( $wpcloudy_enable_geolocation == 'yes' && $_COOKIE['wpc-manualGeolocation']=='1' && $wpcloudy_enable_geolocation_custom_field != 'yes' )  {
            	$use_errors = libxml_use_internal_errors(true);
            	$myweather = @simplexml_load_string(wp_remote_fopen("http://api.openweathermap.org/data/2.5/forecast/weather?id=$wpcloudy_select_city_id&mode=xml&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key"));
            	if (!$myweather) {
          			_e('Unable to retrieve weather data','wp-cloudy');
				}
				libxml_clear_errors();
        		...
        		if ($wpc_advanced_set_disable_cache == '1') {
              		$use_errors = libxml_use_internal_errors(true);
              		$myweather = @simplexml_load_string(wp_remote_fopen("http://api.openweathermap.org/data/2.5/forecast/weather?q=$wpcloudy_city,$wpcloudy_state_name,$wpcloudy_country_code&mode=xml&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key", array( 'timeout' => 10)));
              		if (!$myweather) {
	          			_e('Unable to retrieve weather data','wp-cloudy');
					}
					libxml_clear_errors();
					libxml_use_internal_errors($use_errors);
	              	...
	              	if( $wpcloudy_enable_geolocation == 'yes' && $wpcloudy_enable_geolocation_custom_field == 'yes' && $wpcloudy_custom_field_city_value !='' && $wpcloudy_custom_field_country_value !='' && false === ( $myweather = @simplexml_load_string(get_transient( 'myweather_'.$wpc_id.'_'.$wpc_get_the_ID ) ) ) )  { 
                		$use_errors = libxml_use_internal_errors(true);
                		$myweather = wp_remote_fopen("http://api.openweathermap.org/data/2.5/forecast/weather?q=$wpcloudy_custom_field_city_value,$wpcloudy_custom_field_country_value&mode=xml&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key");
	                	if ($myweather) {
							set_transient( 'myweather_'.$wpc_id.'_'.$wpc_get_the_ID, (string)$myweather, $wpc_advanced_set_cache_time * MINUTE_IN_SECONDS );		                	
	                		$myweather = @simplexml_load_string(get_transient( 'myweather_'.$wpc_id.'_'.$wpc_get_the_ID ) );
						}
						libxml_clear_errors();
						libxml_use_internal_errors($use_errors);
	              	}
	              	elseif( $wpcloudy_enable_geolocation == 'yes' && $wpcloudy_enable_geolocation_custom_field == 'yes' && $wpcloudy_custom_field_lat_value !='' && $wpcloudy_custom_field_lon_value !='' && false === ( $myweather = @simplexml_load_string(get_transient( 'myweather_'.$wpc_id.'_'.$wpc_get_the_ID ) ) ) )  {  
	                	$use_errors = libxml_use_internal_errors(true);
	                	$myweather = wp_remote_fopen("http://api.openweathermap.org/data/2.5/forecast?lat=$wpcloudy_custom_field_lat_value&lon=$wpcloudy_custom_field_lon_value&mode=xml&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key");
		                if ($myweather) {
		                	set_transient( 'myweather_'.$wpc_id.'_'.$wpc_get_the_ID, (string)$myweather, $wpc_advanced_set_cache_time * MINUTE_IN_SECONDS );
		                	$myweather = @simplexml_load_string(get_transient( 'myweather_'.$wpc_id.'_'.$wpc_get_the_ID ) );
	                	}
	                ...
	                && false === ( $myweather = @simplexml_load_string(get_transient( 'myweather_'.$wpc_id ) ) ) ) {
	                	$use_errors = libxml_use_internal_errors(true);
	                	$myweather = wp_remote_fopen("http://api.openweathermap.org/data/2.5/forecast/weather?q=$wpcloudy_city,$wpcloudy_state_name,$wpcloudy_country_code&mode=xml&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key");
		                if (!$myweather) {
		                }else {
        ...
        if ($wpcloudy_forecast && !$wpcloudy_forecast_nd == '' ) {  
      		if( $wpcloudy_enable_geolocation == 'yes' && $_COOKIE['wpc-detectGeolocation']=='1' && $wpcloudy_enable_geolocation_custom_field != 'yes' ) {
            	$use_errors = libxml_use_internal_errors(true);
            	$myweather_sevendays = @simplexml_load_string(wp_remote_fopen("http://api.openweathermap.org/data/2.5/forecast/daily?lat=$wpcloudy_lat&lon=$wpcloudy_lon&mode=xml&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key&cnt=16"));
            	if (!$myweather_sevendays) {
          			_e('Unable to retrieve weather data','wp-cloudy');
                } 
            	libxml_clear_errors();
				libxml_use_internal_errors($use_errors);
        	...
        	elseif( $wpcloudy_enable_geolocation == 'yes' && $_COOKIE['wpc-manualGeolocation']=='1' && $wpcloudy_enable_geolocation_custom_field != 'yes' )  {
            	$use_errors = libxml_use_internal_errors(true);
            	$myweather_sevendays = @simplexml_load_string(wp_remote_fopen("http://api.openweathermap.org/data/2.5/forecast/daily?id=$wpcloudy_select_city_id&mode=xml&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key&cnt=16"));
            	if (!$myweather_sevendays) {
          			_e('Unable to retrieve weather data','wp-cloudy');
                } 
            	libxml_clear_errors();
            	...
            	if ($wpc_advanced_set_disable_cache == '1') {
              		$use_errors = libxml_use_internal_errors(true);
              		$myweather_sevendays = @simplexml_load_string(wp_remote_fopen("http://api.openweathermap.org/data/2.5/forecast/daily?q=$wpcloudy_city,$wpcloudy_state_name,$wpcloudy_country_code&mode=xml&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key&cnt=16", array( 'timeout' => 10)));  
              		if (!$myweather_sevendays) {
	          			_e('Unable to retrieve weather data','wp-cloudy');
	                } 
	            	libxml_clear_errors();
					libxml_use_internal_errors($use_errors);
	              	...
	              	if( $wpcloudy_enable_geolocation == 'yes' && $wpcloudy_enable_geolocation_custom_field == 'yes' && $wpcloudy_custom_field_city_value !='' && $wpcloudy_custom_field_country_value !='' && false === ( $myweather_sevendays = @simplexml_load_string(get_transient( 'myweather_sevendays_'.$wpc_id.'_'.$wpc_get_the_ID ) ) ) )  {             
	                	$use_errors = libxml_use_internal_errors(true);
	                	$myweather_sevendays = wp_remote_fopen("http://api.openweathermap.org/data/2.5/forecast/daily?q=$wpcloudy_custom_field_city_value,$wpcloudy_custom_field_country_value&mode=xml&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key&cnt=16");
	                	if ($myweather_sevendays) {
		                	set_transient( 'myweather_sevendays_'.$wpc_id.'_'.$wpc_get_the_ID, (string)$myweather_sevendays, $wpc_advanced_set_cache_time * MINUTE_IN_SECONDS );	                	
	                		$myweather_sevendays = @simplexml_load_string(get_transient( 'myweather_sevendays_'.$wpc_id.'_'.$wpc_get_the_ID ) );
		                }
		            	libxml_clear_errors();
						libxml_use_internal_errors($use_errors);
	              	}
	              	elseif( $wpcloudy_enable_geolocation == 'yes' && $wpcloudy_enable_geolocation_custom_field == 'yes' && $wpcloudy_custom_field_lat_value !='' && $wpcloudy_custom_field_lon_value !='' && false === ( $myweather_sevendays = @simplexml_load_string(get_transient( 'myweather_sevendays_'.$wpc_id.'_'.$wpc_get_the_ID ) ) ) )  {              
	                	$use_errors = libxml_use_internal_errors(true);
	                	$myweather_sevendays = wp_remote_fopen("http://api.openweathermap.org/data/2.5/forecast/daily?lat=$wpcloudy_custom_field_lat_value&lon=$wpcloudy_custom_field_lon_value&mode=xml&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key&cnt=16");
	                	if ($myweather_sevendays) {
		                	set_transient( 'myweather_sevendays_'.$wpc_id.'_'.$wpc_get_the_ID, (string)$myweather_sevendays, $wpc_advanced_set_cache_time * MINUTE_IN_SECONDS );
		                	$myweather_sevendays = @simplexml_load_string(get_transient( 'myweather_sevendays_'.$wpc_id.'_'.$wpc_get_the_ID ) );
	              		}
	                ...
	                && false === ( $myweather_sevendays = @simplexml_load_string(get_transient( 'myweather_sevendays_'.$wpc_id ) ) ) ) {  
	             		$use_errors = libxml_use_internal_errors(true);
	                	$myweather_sevendays = wp_remote_fopen("http://api.openweathermap.org/data/2.5/forecast/daily?q=$wpcloudy_city,$wpcloudy_state_name,$wpcloudy_country_code&mode=xml&units=$wpcloudy_unit&APPID=$wpc_advanced_api_key&cnt=16");
	                	if (!$myweather_sevendays) {
		...
		$owm_link         						= '<a href="http://openweathermap.org/city/'.$myweather_current->city[0]['id'].'" target="_blank" title="'.__('Full weather on OpenWeatherMap','wp-cloudy').'">'.__('Full weather','wp-cloudy').'</a>'; 
		$last_update      						= __('Last update: ','wp-cloudy').date("M-j-Y, H:i", strtotime($myweather_current->lastupdate[0]['value'])); 
		...
		if ($wpcloudy_date_temp == 'no') {
			$today_month_feed = date('m', strtotime($myweather_current->lastupdate[0]['value']));
		...
		if( $wpcloudy_hour_forecast && !$wpcloudy_hour_forecast_nd =='' ) {  
			$hour_temp_0      			= (ceil($myweather->forecast[0]->time[0]->temperature[0]['value']));
		  	...
		  	while ($i<=5) {
		    	$hour_time_[$i]       		= (string)date($wpcloudy_date_php_hours, strtotime($myweather->forecast[0]->time[$i]['from'])+60*$utc_time_wp);
		...
		if ($wpcloudy_forecast && !$wpcloudy_forecast_nd == '' ) { 
			...
			while ($i<=15) {
				$forecast_day_feed      = strftime('%w', strtotime($myweather_sevendays->forecast[0]->time[$i]['day']));
				    ...
				    $forecast_rain_[$i]   		= $myweather_sevendays->forecast[0]->time[$i]->precipitation[0]['value'];
			...
			if ($wpcloudy_unit == 'metric') {
				...
				$time_wind_speed_unit = 'm/h';
			}
			$display_wind = '
			<div class="wind">'. __( 'Wind', 'wp-cloudy' ) .'<span class="wpc-highlight">'. $time_wind_direction .' '. $time_wind_speed .' '.$time_wind_speed_unit.'</span></div>
			';
			$display_humidity = '
			    	...
			    	$display_forecast_[$i] .= '  
			       		<div class="'. $wpcloudy_class_days[$i].'">
			          		...
			          		if ($wpcloudy_forecast_precipitation ) {
			          			if ($forecast_rain_[$i]) {
			          				$display_forecast_[$i] .= '<div class="rain">'. $forecast_rain_[$i] .' mm</div>';
			          			} else {
		...
		$wpcloudy_current_weather   						=   get_bypass_display_current_weather($attr);
		$wpcloudy_weather       							=   get_bypass_display_weather($attr);
		$wpcloudy_wind          							=   get_bypass_display_wind($attr);
		$wpcloudy_humidity        							=   get_bypass_display_humidity($attr);
		$wpcloudy_pressure        							=   get_bypass_display_pressure($attr);
		$wpcloudy_cloudiness      							=   get_bypass_display_cloudiness($attr);
		$wpcloudy_precipitation     						=   get_bypass_display_precipitation($attr);
		$wpcloudy_temperature_min_max 						= 	get_bypass_temp($attr);
		$wpcloudy_size          							= 	get_bypass_size($attr);
...
function wpcloudy_notice() {    
	...
	if ( is_plugin_active( 'wp-cloudy/wpcloudy.php' ) && $wpc_advanced_api_key['wpc_advanced_api_key'] =='') {
	    ?>
	    <div class="error notice">
	    
	        <p><a href="<?php echo get_bloginfo('wpurl'); ?>/wp-admin/admin.php?page=wpc-settings-admin#tab_advanced"><?php _e( 'WP Cloudy: Please enter your own OpenWeatherMap API key to avoid limits requests.', 'wpui' ); ?></a></p>
	    </div>
	    <?php
	}
}
