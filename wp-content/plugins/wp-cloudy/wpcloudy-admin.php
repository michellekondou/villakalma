...
class wpc_options
{
    ...
    public function create_admin_page()
    {
        ...
        <div id="wpcloudy-header">
            <div id="wp-admin-ui-banner">
                <span class="wp-admin-ui-txt"><?php _e('The best freemium plugin to customize WordPress administration in seconds.','wp-cloudy'); ?></span>
                <a target="_blank" href="http://wpadminui.net/?utm_source=WP%Admin%20UI&utm_medium=global-settings-sidebar&utm_term=download&utm_campaign=WP%20Admin%20UI" class="button-primary more-info-link"><?php _e('Download for free','wp-cloudy'); ?></a>
            </div>
    ...
    {        
		...
		add_settings_field(
            ...
            __("16-Day Forecast","wp-cloudy"), // Title 
        ...
        add_settings_field(
            'wpc_map_js', // ID
            __("Load JS/CSS from:","wp-cloudy"), // Title
	...
	public function wpc_display_forecast_callback()
    {
		...
		if ('1' == $check) echo 'checked="yes"'; 
		echo ' value="1"/>';
		echo '<label for="wpc_display_forecast">'. __( 'Display 16-day Forecast on all weather?', 'wp-cloudy' ) .'</label>';
	...
	public function wpc_display_forecast_nd_callback()
    {
		...
		echo '<option '; 
		if ('6' == $selected) echo 'selected="selected"'; 
		echo ' value="6">'. __( '6 days', 'wp-cloudy' ) .'</option>';
		echo '<option '; 
		if ('7' == $selected) echo 'selected="selected"'; 
		echo ' value="7">'. __( '7 days', 'wp-cloudy' ) .'</option>';
		echo '<option '; 
		if ('8' == $selected) echo 'selected="selected"'; 
		echo ' value="8">'. __( '8 days', 'wp-cloudy' ) .'</option>';
		echo '<option '; 
		if ('9' == $selected) echo 'selected="selected"'; 
		echo ' value="9">'. __( '9 days', 'wp-cloudy' ) .'</option>';
		echo '<option '; 
		if ('10' == $selected) echo 'selected="selected"'; 
		echo ' value="10">'. __( '10 days', 'wp-cloudy' ) .'</option>';
		echo '<option '; 
		if ('11' == $selected) echo 'selected="selected"'; 
		echo ' value="11">'. __( '11 days', 'wp-cloudy' ) .'</option>';
		echo '<option '; 
		if ('12' == $selected) echo 'selected="selected"'; 
		echo ' value="12">'. __( '12 days', 'wp-cloudy' ) .'</option>';
		echo '<option '; 
		if ('13' == $selected) echo 'selected="selected"'; 
		echo ' value="13">'. __( '13 days', 'wp-cloudy' ) .'</option>';
		echo '<option '; 
		if ('14' == $selected) echo 'selected="selected"'; 
		echo ' value="14">'. __( '14 days', 'wp-cloudy' ) .'</option>';
		echo '<option '; 
		if ('15' == $selected) echo 'selected="selected"'; 
		echo ' value="15">'. __( '15 days', 'wp-cloudy' ) .'</option>';
		echo '</select>';
    ...
    public function wpc_display_fluid_width_callback()
    {
        ...
        esc_attr( $this->options['wpc_display_fluid_width'])
        );
    } 
    ...
    public function wpc_advanced_api_key_callback()
    {
        ...
        echo '<br /><br /><span class="dashicons dashicons-editor-help"></span>'.__('Strongly recommended: ','wp-cloudy').'<a href="http://openweathermap.org/appid" target="_blank">'.__('Strongly recommended: get your free OWM API key here','wp-cloudy').'</a>';
    }	
    ...
    public function wpc_support_info_callback()
    {
		echo '
			...
			<p><a href="http://wordpress.org/plugins/wp-cloudy/" target="_blank" title="'. __("WP Cloudy Forum on WordPress.org", "wp-cloudy").'">'. __("WP Cloudy Forum on WordPress.org", "wp-cloudy").'</a></p>
			';
...
function wpc_help_tab() {
    ...
    $screen->add_help_tab( array(
        ...
        'content'   => '<p>' . __( 'Follow this video to setup WP Cloudy:' ) . '<br><iframe width="560" height="315" src="https://www.youtube.com/embed/mRF_3VOz_OE?rel=0&amp;showinfo=0" frameborder="0" allowfullscreen></iframe></p>',
    ));
    $screen->add_help_tab( array(
        'id'    => 'wpc_help_tab2',
        'title' => __('Create your first weather'),
        'content'   => '<p>' . __( 'Follow this video to create your first weather with WP Cloudy:' ) . '<br><iframe width="560" height="315" src="https://www.youtube.com/embed/xv4lrgsWkkk?rel=0&amp;showinfo=0" frameborder="0" allowfullscreen></iframe></p>',
    ));
}
?>